...
public class PantallaSeleccionPersonaje implements Screen {
    ...
    private void inicializarPersonajes() {
        // Tomamos los personajes del registro global (ya inicializados)
        List<Personaje> lista = com.miempresa.mijuego.config.PersonajeRegistry.getInstance().getPersonajes();

        // Convertimos la lista a array (para mantener el resto del código igual)
        personajesDisponibles = lista.toArray(new Personaje[0]);
    }
    ...
    private void confirmarSeleccion() {
        ...
        Jugador.asignarPaisesIniciales(jugador);
